...
def question_1(tab)
  puts "Il y a " + tab.length.to_s + " handles dans le tableau."
end

def question_2(tab)
  value_min = tab[0]
  tab.each {|value|
    if value.length < value_min.length
      value_min = value
    end}
    puts "La pus petite handle est "+ value_min +"."
end
...
def question_3(tab)
  ...
  tab.each {|value|
    ...
    puts "Il y a " + count.to_s + " handles de 5 caractères."
  end
...
def question_4(tab)
  value_up = 0
      ...
      puts "Voici le nombre de handles qui commencent par une majuscule : " +value_up.to_s 
end

def question_5(tab)
  alpha = tab.sort { |a, b| a.downcase <=> b.downcase }
...
def question_6(tab)
  ...
  puts "Voici la liste par longueur de handle :"
  puts long
end
...
def question_7(tab)
  ...
  position = position + 1
  puts "@epenser se situe à la position : " + position.to_s
end
